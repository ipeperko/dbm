#ifndef DBM_POSTGRES_SESSION_HPP
#define DBM_POSTGRES_SESSION_HPP

#include <dbm/session.hpp>

namespace dbm {

class DBM_EXPORT postgres_session : public session
{
public:
    postgres_session();
    //postgres_session(const postgres_session& oth);
    //postgres_session(postgres_session&& oth) noexcept;
    //postgres_session& operator=(const postgres_session& oth);
    //postgres_session& operator=(postgres_session&& oth) noexcept;
    ~postgres_session() override;

    std::unique_ptr<session> clone() const override;

    void init(const std::string& host_name, const std::string& user, const std::string& password, unsigned int port, const std::string& db_name);
...
#endif //DBM_POSTGRES_SESSION_HPP
